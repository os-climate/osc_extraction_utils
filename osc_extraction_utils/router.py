...
class Router:
    ...
    def run_router(self):
        ...
        self._send_payload_to_server_address_with_node(
            self._extraction_server_address, "extract"
        )
        self._send_payload_to_server_address_with_node(
            self._extraction_server_address, "curate"
        )
    ...
    def _send_payload_to_server_address_with_node(
        self, server_address: str, node: str
    ) -> None:
        response: requests.Response = requests.get(
            f"{server_address}/{node}", params=self._payload
        )
        print(response.text)
    ...
    def _check_extraction_server_is_live(self) -> None:
        response: requests.Response = requests.get(
            f"{self._extraction_server_address}/liveness"
        )
        if response.status_code == 200:
    ...
    def _define_payload(self) -> None:
        self._payload = {
            "project_name": self._main_settings.general.project_name,
            "mode": "train",
        }
    ...
    def _check_inference_server_is_live(self) -> None:
        response: requests.Response = requests.get(
            f"{self._inference_server_address}/liveness"
        )
        if response.status_code == 200:
    ...
    def _check_for_train_relevance_training_and_send_request(self) -> None:
        ...
        if self._main_settings.train_relevance.train:
            self._send_payload_to_server_address_with_node(
                self._inference_server_address, "train_relevance"
            )
        else:
            print(
    ...
    def _check_for_kpi_training_and_send_request(self) -> None:
        if self._main_settings.train_kpi.train:
            self._send_payload_to_server_address_with_node(
                self._inference_server_address, "infer_relevance"
            )
            self._check_for_generate_text_3434()
            print(
                "Next we start the training of the inference model. This may take some time."
            )
            self._send_payload_to_server_address_with_node(
                self._inference_server_address, "train_kpi"
            )
        else:
            print(
