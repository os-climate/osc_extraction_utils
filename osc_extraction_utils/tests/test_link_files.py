...
from osc_extraction_utils.utils import link_extracted_files, link_files


@pytest.fixture(autouse=True)
def path_folders_required_linking(
...
def test_link_extracted_files_result(path_folder_temporary: Path):
    ...
    result = link_extracted_files(
        str(path_folder_source),
        str(path_folder_source_pdf),
...
def test_link_extracted_files_copy(path_folder_temporary: Path):
    ...
    for i in range(10):
        ...
        assert path_current_file.exists() is not True

    link_extracted_files(
        str(path_folder_source),
        str(path_folder_source_pdf),
        str(path_folder_destination),
    )
