...
def test_download_inference_related_files_from_s3(merger: Merger):
    string_path_from_s3: str = str(
        Path(merger.s3_settings.prefix)
        / merger.project_paths.string_project_name
        / "data"
        / "output"
        / "RELEVANCE"
        / "Text"
    ...
    mocked_s3_communication.download_files_in_prefix_to_dir.assert_called_with(
        string_path_from_s3, string_path_local_folder
    )


def test_upload_to_s3(merger: Merger):
    path_local_file: Path = (
        Path(merger.s3_settings.prefix)
        / merger.project_paths.string_project_name
        / "data"
        / "interim"
...
def test_write_output(merger: Merger, path_folder_temporary: Path):
