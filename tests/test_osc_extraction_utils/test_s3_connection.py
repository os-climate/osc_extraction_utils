...
from osc_extraction_utils.core_utils import download_data_from_s3_main_bucket_to_local_folder_if_required, upload_data_from_local_folder_to_s3_interim_bucket_if_required
from osc_extraction_utils.settings import get_s3_settings, MainSettings, Settings, General
...
def test_download_data_from_s3_main_bucket_to_local_folder_if_required(s3_usage: bool, main_settings: Settings):
    ...
    with patch.object(main_settings.general, 's3_usage', s3_usage):
        download_data_from_s3_main_bucket_to_local_folder_if_required(mocked_s3_bucket, mocked_path_s3, 
                                                                      mocked_path_local, main_settings)
...
def test_upload_data_from_local_folder_to_s3_interim_bucket_if_required(s3_usage: bool, main_settings: Settings):
    ...
    with (patch.object(main_settings.general, 's3_usage', s3_usage)):
        upload_data_from_local_folder_to_s3_interim_bucket_if_required(mocked_s3_bucket, mocked_path_local, 
                                                                       mocked_path_s3, main_settings)
